...
const JetukuBoliniAga = () => {
  ...
  const [jetukuSections, setJetukuSections] = useState([]);
  ...
  useEffect(() => {
    const fetchJetukuData = async () => {
      try {
        ...
        setJetukuSections(response.data);
      } catch (err) {
  ...
  const handleOpenModal = (data) => {
    setSelectedJetuku({
      title: data?.title || "যেটুকু বলিনি আগে",
  ...
  return (
    <section className="jetukuboliniaga" id={jetukuMenu?.path || "jetukuboliniaga"}>
      <div className="container">
        ...
        {jetukuSections.map((jetukuData, index) => (
          <div key={jetukuData._id || index} className="mb-5">

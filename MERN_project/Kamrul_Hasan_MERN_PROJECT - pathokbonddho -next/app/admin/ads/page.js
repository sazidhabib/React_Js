...
const AdForm = ({ ad, onClose, onSuccess }) => {
    ...
    const handleInputChange = (e) => {
        ...
        setFormData(prev => ({ ...prev, [name]: type === 'checkbox' ? checked : (name === 'name' ? value : value), ...(name === 'name' && { slug: value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)+/g, '') }) }));
...
const AdsDashboard = () => {
    ...
    useEffect(() => { fetchAds(); }, [pagination.page, filters]);
    ...
    if (!user) return <div className="container mt-4"><div className="alert alert-warning">Please login to manage ads.</div></div>;
                    ...
                    <Card.Body>
                        ...
                        <Table striped bordered hover responsive>
                            <thead className="table-dark">
                                <tr>
                                    <th><Form.Check checked={ads.length > 0 && selectedAds.length === ads.length} onChange={(e) => setSelectedAds(e.target.checked ? ads.map(a => a.id) : [])} /></th>
                                    <th>Ad</th>
                                    <th>Type</th>
                                    <th>Position</th>
                                    <th>Status</th>
                            ...
                            <tbody>
                                {loading ? <tr><td colSpan="7" className="text-center py-5"><Spinner animation="border" /></td></tr> : ads.map(ad => (
                                    <tr key={ad.id}>
                                        ...
                                        <td>{ad.currentImpressions || 0} / {ad.clickCount || 0} ({ad.currentImpressions ? ((ad.clickCount / ad.currentImpressions) * 100).toFixed(1) : 0}%)</td>
                                        <td>
                                            <div className="d-flex gap-1">
                                                <Button variant="outline-primary" size="sm" onClick={() => { setEditingAd(ad); setShowForm(true); }}>Edit</Button>
                                                <Button variant="outline-danger" size="sm" onClick={() => handleDelete(ad.id)}>Delete</Button>
                                            </div>
                                        </td>
                                    </tr>
                                ))}

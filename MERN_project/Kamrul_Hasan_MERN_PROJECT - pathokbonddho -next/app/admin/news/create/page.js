...
const NewsCreate = () => {
    ...
    const handleEditorImageSelect = (photo) => {
        ...
        setModals(prev => ({ ...prev, format: true, editor: false }));
    };
            ...
            <Modal.Body>
                ...
                <Row className="g-2">
                    {photos.filter(p => (selectedAlbum === 'all' || p.albumId == selectedAlbum) && (p.filename.toLowerCase().includes(imageSearch.toLowerCase()))).map(p => (
                        <Col md={3} key={p.id}>
                            <Card className="h-100 cursor-pointer" onClick={() => { if(type === 'editor') handleEditorImageSelect(p); else { setSelectedImages({...selectedImages, [type]: p}); setFiles({...files, [type]: null}); onHide(); } }}>
        ...
        <Container fluid className="py-4">
            <Form onSubmit={handleAction}>
                <div className="d-flex justify-content-between mb-3">
                    ...
                    <Button type="submit" disabled={loading}>{loading ? 'Saving...' : 'Publish News'}</Button>
                </div>
                ...
                <Row>
                    <Col md={8}>
                        <Card className="mb-3"><Card.Body>
                            ...
                            <Form.Group className="mb-3"><Form.Label>Headline (Bangla)</Form.Label><Form.Control value={formData.newsHeadlineBangla} onChange={e => setFormData({...formData, newsHeadlineBangla: e.target.value})} /></Form.Group>
                            ...
                            <Form.Group className="mb-3"><Form.Label>Short Description</Form.Label><WYSIWYGEditor ref={editorRefs.shortDescription} value={formData.shortDescription} onChange={v => setFormData({...formData, shortDescription: v})} height={150} onImageClick={() => { setActiveEditor('shortDescription'); setModals({...modals, editor: true}); }} /></Form.Group>
                            <Form.Group className="mb-3"><Form.Label>Main Content *</Form.Label><WYSIWYGEditor ref={editorRefs.content} value={formData.content} onChange={v => setFormData({...formData, content: v})} height={400} onImageClick={() => { setActiveEditor('content'); setModals({...modals, editor: true}); }} /></Form.Group>
                        </Card.Body></Card>
                    </Col>
                    <Col md={4}>
                        <Card className="mb-3"><Card.Body>
                            ...
                            <Form.Group className="mb-3"><Form.Label>Status</Form.Label><Form.Select value={formData.status} onChange={e => setFormData({...formData, status: e.target.value})}><option value="draft">Draft</option><option value="published">Published</option><option value="scheduled">Scheduled</option></Form.Select></Form.Group>
                            {formData.status === 'scheduled' && <Form.Control type="datetime-local" className="mb-3" value={formData.newsSchedule} onChange={e => setFormData({...formData, newsSchedule: e.target.value})} />}
            ...
            <Modal show={modals.format} onHide={() => setModals({...modals, format: false})} centered>
                ...
                <Modal.Body className="text-center">
                    ...
                    <div className="d-flex flex-column gap-2">
                        <Button variant="outline-primary" onClick={() => handleFormatConfirm({format: 'full-width'})}>Full Width</Button>
                        <Button variant="outline-primary" onClick={() => handleFormatConfirm({format: 'left-aligned'})}>Left Aligned</Button>
                        <Button variant="outline-primary" onClick={() => handleFormatConfirm({format: 'right-aligned'})}>Right Aligned</Button>
                        <Button variant="outline-primary" onClick={() => handleFormatConfirm({format: 'full-width-captioned', caption: 'Image caption'})}>Captioned</Button>
                    </div>
                </Modal.Body>
            </Modal>

            {/* Author Selection Tool */}
                ...
                <Modal.Body>
                    ...
                    <div style={{maxHeight: '300px', overflowY: 'auto'}}>
                        {authors.filter(a => a.name.toLowerCase().includes(searches.author.toLowerCase())).map(a => (
                            <div key={a.id} className="p-2 border-bottom cursor-pointer hover-bg-light" onClick={() => { setFormData({...formData, authorId: a.id, authorName: a.name}); setDropdowns({...dropdowns, author: false}); }}>{a.name}</div>
                        ))}
                    </div>
                </Modal.Body>
            </Modal>

                    ...
                    <Row className="g-2">
                        {searchedPhotos.length === 0 ? <div className="text-center text-muted py-4">No images found</div> : searchedPhotos.map(photo => (
                            <Col md={3} key={photo.id}>
                                <Card className="h-100 cursor-pointer" onClick={() => { if (galleryMode) { setTargetGalleryIndex(targetGalleryIndex); handleGalleryImageSelect(photo); } else onSelect(photo); }} style={{ cursor: 'pointer' }}>

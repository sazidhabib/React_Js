...
class SubCategoryController extends Controller
{
    ...
    public function create(){
        $default_language = Language::where('is_default',1)->first();
        $categories = Category::where('parent_id','=',NULL)->where('language_id',$default_language->id)->get();
        $languages = Language::orderBy('id','desc')->get();  // Change $datas to $languages
        return view('admin.subcategory.create',compact('categories','languages'));
    }
    ...
    public function edit($id){
        ...
        return view('admin.subcategory.edit', compact('data', 'languages'));
    }
    ...
    public function languageOnUpdate($x, $y){
        $datas = Language::find($x)->categories()->where('parent_id', '=', NULL)->get();
        $subCategory = Category::find($y);
        $output = '<option value="">Please Select a Category *</option>';
        foreach($datas as $data){
            $selected = ($data->id == $subCategory->parent_id) ? 'selected' : '';
            $output .= '<option value="'.$data->id.'" '.$selected.'>'.e($data->title).'</option>';
        }
        return $output;
    }

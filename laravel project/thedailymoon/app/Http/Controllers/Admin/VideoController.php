...
class VideoController extends Controller
{
    ...
    public function index(){
        ...
        return view('admin.video.index', compact('categories'));
    }
    ...
    public function datatables(Request $request){
        ...
        // Start query for video posts only
        $query = Post::where('post_type', 'video')
                     ->where('status', 'true')
                     ->orderBy('id', 'desc');
        ...
        return DataTables::of($datas)
            ...
            ->editColumn('post_type', function(Post $data) {
                return '<span class="badge badge-secondary">video</span>';
            })
    ...
    public function store(Request $request){
        ...
        if($video = $request->file('video')){
            ...
            if (!file_exists(public_path('assets/videos'))) {
                mkdir(public_path('assets/videos'), 0755, true);
            }
            $video->move(public_path('assets/videos'), $videoName);
        ...
        if($request->embed_video){
            ...
            if(isset($data->video) && $data->video && file_exists(public_path('assets/videos/'.$data->video))){
                @unlink(public_path('assets/videos/'.$data->video));
            }
            $input['video'] = '';
        }
    ...
    public function update(Request $request,$id){
        ...
        if($request->hasFile('video')){
            ...
            $videoName = time().$video->getClientOriginalName();
            $video->move(public_path('assets/videos'), $videoName);
            if($data->video && file_exists(public_path('assets/videos/'.$data->video))){
                @unlink(public_path('assets/videos/'.$data->video));
            }
            $input['video'] = $videoName;
            $input['embed_video'] = '';

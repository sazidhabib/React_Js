...
<div class="content-area">
    <div class="mr-breadcrumb">
        <div class="row">
            <div class="col-lg-12">
                <h4 class="heading">{{ __('Audio Posts') }}</h4>
                <ul class="links">
                    <li>
                        <a href="{{ route('admin.dashboard') }}">{{ __('Dashboard') }} </a>
                    </li>
                    <li>
                        <a href="{{ route('slider.index') }}">{{ __('Audio Posts') }}</a>
                    </li>
                </ul>
            </div>
        </div>
                ...
                <div class="d-inline-block">
                    ...
                    <select id="category_id" class="form-control d-inline-block" style="width: auto; display: inline-block;">
                        ...
                        @foreach ($categories as $category)
                            <option data-href="{{ route('audio.datatables') }}?category={{ $category->id }}" value="{{ $category->id }}">{{ $category->title }}</option>
                        @endforeach
                    </select>
                </div>
...
<div class="modal fade-scale" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">{{ __('You are about to delete this Audio Post.') }}</p>
                <p class="text-center">{{ __('Do you want to proceed?') }}</p>
            </div>
            <div class="modal-footer justify-content-center">
                <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('Cancel') }}</button>
...
$(document).ready(function() {
    ...
    $(document).on("change", "#category_id", function() {
        ...
        if (url && selectedOption.val() !== '') {
            ...
            table.ajax.url('{{ route("audio.datatables") }}').load();
        }
        toggleClearButton();
    });

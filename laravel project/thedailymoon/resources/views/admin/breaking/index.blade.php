                ...
                <div class="d-inline-block">
                    ...
                    <select id="category_id" class="form-control d-inline-block" style="width: auto; display: inline-block;">
                        ...
                        @foreach ($categories as $category)
                            <option data-href="{{ route('breaking.datatables') }}?category={{ $category->id }}" value="{{ $category->id }}">{{ $category->title }}</option>
                        @endforeach
                    </select>
                </div>
...
<div class="modal fade-scale" id="confirm-delete" tabindex="-1" role="dialog" aria-labelledby="modal1" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <p class="text-center">{{ __('You are about to delete this Breaking Post.') }}</p>
                <p class="text-center">{{ __('Do you want to proceed?') }}</p>
            </div>
            <div class="modal-footer justify-content-center">
                <button type="button" class="btn btn-default" data-dismiss="modal">{{ __('Cancel') }}</button>
...
$(document).ready(function() {
    ...
    $(document).on("change", "#category_id", function() {
        ...
        if (url && selectedOption.val() !== '') {
            ...
            table.ajax.url('{{ route("breaking.datatables") }}').load();
        }
        toggleClearButton();
    });

                ...
                <div class="product-description">
                    ...
                    <div class="gocover" style="background: url({{asset('assets/images/'.$gs->admin_loader)}}) no-repeat scroll center center rgba(45, 45, 45, 0.5);"></div>
                     <form id="geniusformdata" action="{{ route('rss.update',$data->id)}}" method="POST" enctype="multipart/form-data">
                        ...
                        @endphp
                        <input type="hidden" name="language_id" id="rss_language_update_id" value="{{ $data->language_id ? $data->language_id : $default_language->id }}">
                        ...
                        <div class="row">
                            <div class="col-lg-12">
                                <button class="addProductSubmit-btn"
                                    type="submit">{{ __('Update Rss Feed') }}</button>
                            </div>
                        </div>
...
(function($) {
    ...
    $(document).ready(function() {
        ...
        if (language && rssId) {
            var url = "{{ url('/') }}/admin/rss/categoryUpdate/" + language + "/" + rssId;

...
//     $file = public_path('/project/license.txt');
//     if (file_exists($file)) {
//         unlink($file);
//         return 'System deactivated successfully! <a href="' . url('/admin') . '">Go back to admin</a>';
//     }
//     return 'System is already deactivated! <a href="' . url('/admin') . '">Go back to admin</a>';
// });

Route::get('/admin/deactivation', 'Admin\DashboardController@deactivation')->name('admin-deactivation');
...
Route::prefix('admin')->group(function(){
    ...
    Route::group(['middleware' => 'permissions:categories'], function () {
        ...
        Route::get('/subcategory/language/update/{x}/{y}', 'Admin\SubCategoryController@languageOnUpdate')->name('subcategory.language.update');
    ...
    Route::group(['middleware' => 'permissions:add_post'], function () {
        ...
        //-------------Post Audio area-----------
        Route::get('/audios','Admin\AudioController@index')->name('audio.index');
        Route::get('/audio/datatables', 'Admin\AudioController@datatables')->name('audio.datatables');
        Route::get('/audio/category-filter/language/{id}', 'Admin\AudioController@categoryFilter')->name('audio.category.filter');
        ...
        Route::get('/video/datatables', 'Admin\VideoController@datatables')->name('video.datatables');
        Route::get('/video/category-filter/language/{id}', 'Admin\VideoController@categoryFilter')->name('video.category.filter');

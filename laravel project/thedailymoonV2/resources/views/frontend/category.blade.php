            ...
            @else
                <div class="card" style="border-radius: 0; border: 1px solid var(--bbc-border);">
                    <div class="card-body" style="padding: 30px; text-align: center;">
                        <p style="color: var(--bbc-red); font-weight: 700; margin: 0;">{{__('This Category has no news.')}}</p>
                    </div>
                </div>
            @endif  
        </div>
        ...
        <div class="col-md-4 col-sm-4">
            ...
            <div class="bbc-widget">
                ...
                <div id="bbc-tab-archive-latest">
                    @foreach($is_trendings->take(6) as $is_trending)
                        <a href="{{ route('frontend.postBySubcategory.details',[$is_trending->category->slug ?? 'category',$is_trending->slug])}}" class="bbc-list-item">
                            ...
                            <div class="bbc-list-item-title">{{strlen($is_trending->title)>60 ? mb_substr($is_trending->title,0,60,"utf-8").'...' : $is_trending->title}}</div>
                        </a>
                    @endforeach
                </div>
                
                <div id="bbc-tab-archive-popular" style="display: none;">
                    @foreach ($is_recents->take(6) as $is_recent)
                        <a href="{{ route('frontend.postBySubcategory.details',[$is_recent->category->slug ?? 'category',$is_recent->slug])}}" class="bbc-list-item">
                            ...
                            <div class="bbc-list-item-title">{{strlen($is_recent->title)>60 ? mb_substr($is_recent->title,0,60,"utf-8").'...' : $is_recent->title}}</div>
                        </a>
                    @endforeach
                </div>
            </div>

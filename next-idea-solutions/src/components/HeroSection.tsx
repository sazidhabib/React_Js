import { ArrowRight, Play, Zap, Shield, Globe, Cpu, Database, Cloud } from 'lucide-react';
        ...
        <section className="relative min-h-screen flex items-center justify-center overflow-hidden pt-20 pb-0">
            ...
            <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
                ...
                <p className="max-w-2xl mx-auto text-lg md:text-xl text-muted-foreground mb-10">
                    Transform your vision into reality with intelligent software built for the modern world.
                    We deliver precision, speed, and elegance in every line of code.
                </p>
                ...
                <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-16">
                    ...
                    <Button size="lg" variant="outline" className="text-lg px-8 py-6 group glass">
                                ...
                                <div className="grid grid-cols-12 gap-3 md:gap-4 h-[calc(100%-60px)]">
                                    ...
                                    <div className="col-span-2 glass rounded-xl p-2 md:p-3 space-y-3">
                                        {[Zap, Shield, Globe, Database].map((Icon, i) => (
                                            <div key={i} className={`p-2 rounded-lg ${i === 0 ? 'bg-primary/30' : 'hover:bg-primary/10'} transition-colors`}>
                                    ...
                                    <div className="col-span-7 space-y-3 md:space-y-4">
                                        ...
                                        <div className="grid grid-cols-3 gap-2 md:gap-3">
                                            {[
                                                { label: 'Active Users', value: '12.4K', color: 'from-primary to-accent' },
                                                { label: 'Revenue', value: '$84K', color: 'from-chart-2 to-chart-4' },
                                                { label: 'Growth', value: '+24%', color: 'from-chart-3 to-chart-1' },
                                            ].map((stat, i) => (
                                                <div key={i} className="glass rounded-xl p-2 md:p-3">
                                                    <p className="text-[10px] md:text-xs text-muted-foreground">{stat.label}</p>
                                                    <p className={`text-sm md:text-lg font-bold bg-gradient-to-r ${stat.color} bg-clip-text text-transparent`}>{stat.value}</p>
                                                </div>
                                            ))}
                                        </div>
                                    ...
                                    <div className="col-span-3 glass rounded-xl p-2 md:p-3 space-y-2 md:space-y-3">
                                        <div className="flex items-center gap-2 mb-2">
                                            ...
                                            <span className="text-[10px] md:text-xs font-medium">Activity</span>
                                        </div>
                                        {[1, 2, 3, 4].map((_, i) => (
                ...
                <div className="mt-16 md:mt-24">
                    <p className="text-sm text-muted-foreground mb-8">Trusted by companies that build the future</p>
                            ...
                            {[...Array(2)].map((_, setIndex) => (
                                <div key={setIndex} className="flex items-center gap-12 md:gap-16 px-6">
                                    {[
                                        ...
                                        <div key={`${setIndex}-${company.name}`} className="flex items-center gap-3 text-muted-foreground/60 hover:text-muted-foreground transition-colors whitespace-nowrap">
                                            <span className="text-2xl">{company.icon}</span>
                                            <span className="text-lg md:text-xl font-semibold">{company.name}</span>
                                        </div>
                                    ))}
                                </div>
                            ))}
